...
static u8 ChooseWildMonLevel(const struct WildPokemon* wildPokemon)
{
	...
	{
        highestlvl = GetHighestMonLevel(gPlayerParty);
        lowestlvl = GetLowestMonLevel(gPlayerParty);
		min = (highestlvl * 163 / 600) + (lowestlvl * 3 / 8);
		max = (highestlvl * 9 / 16) + (lowestlvl * 163 / 400);
	}
	else
	#endif
